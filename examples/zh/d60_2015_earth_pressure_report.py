...
async def sheet():
    ...
    phi = inputs.phiDeg
    height = inputs.heightH * unit.meter
    depth = inputs.depth * unit.meter
    width = inputs.widthB * unit.meter
    alpha = inputs.alphaDeg
    beta = inputs.betaDeg
    delta = phi / 2
    ...
    vehicleEquivalentHeight = vehicleWheelWeight / vehicleUnitLength / gamma
    show()
    ...
    rows = [
        ...
        ["内摩擦角", "φ", phi, "单位：°"],
        ["填土高度", "H", height, "静土压力与主动土压力计算高度"],
        ["计算深度", "h", depth, "静土压力强度计算点深度"],
        ["墙背倾角", "α", alpha, "单位：°，俯墙背为正"],
        [
            "填土坡角",
            "β",
            beta,
    ...
    if inputs.isColumn:
        ...
        rows.append(["计算宽度或长度", "B", width, "桥台宽度或挡土墙长度"])
    show()
    ...
    xi = 1 - sinPhi
    staticDepthPressure = (xi * gamma * depth).to(unit.kPa)
    staticEarthForce = xi * gamma * height**2 / 2
    ...
    activeEarthForce = width * mu * gamma * height**2 / 2
    activeForcePoint = height / 3
    ...
    hide()
    vehicleApplicable = abs(beta) < 1e-9
    show()
    ...
    if inputs.vehicleWheelWeightPerMeter > 0:
        ...
        if vehicleApplicable:
            vehicleActiveEarthForce = (
                width * mu * gamma * height * (height + 2 * vehicleEquivalentHeight) / 2
            )
            vehicleForcePoint = (
                height
                / 3
                * (height + 3 * vehicleEquivalentHeight)
                / (height + 2 * vehicleEquivalentHeight)
            )
            Table(
                headers=["项目", "结果", "规范式"],
                rows=[
    ...
    lambdaCoeff = math.tan(math.radians(45 - phi / 2)) ** 2
    verticalPressure = (gamma * compactedDepth).to(unit.kPa)
    horizontalPressure = (lambdaCoeff * gamma * compactedDepth).to(unit.kPa)
