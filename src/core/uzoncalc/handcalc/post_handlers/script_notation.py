...
class ScriptNotation(BasePostHandler):
    ...
    def _render_script_notation(
        ...
        """按脚标标记扫描文本，将合法变量上下标块替换为目标格式。"""
        result: list[str] = []
        cursor = 0
        while cursor < len(text):
            parsed_span = self._find_next_script_notation(text, cursor)
            if parsed_span is None:
                result.append(text[cursor:])
                break

            start, end, parsed = parsed_span
    ...
    def _parse_single_script_notation(self, text: str) -> ScriptParseResult | None:
        ...
        parsed_span = self._find_next_script_notation(text, 0)
        if parsed_span is None:
            return None
        start, end, parsed = parsed_span
    ...
    def _find_next_script_notation(
        self, text: str, cursor: int
    ) -> tuple[int, int, ScriptParseResult] | None:
        """查找下一个以 _/^ 为锚点的合法变量上下标块。"""
        while cursor < len(text):
            if text[cursor] not in {"_", "^"}:
                cursor += 1
                continue

            parsed_span = self._read_script_notation_at_mark(text, cursor)
            if parsed_span is not None:
                return parsed_span
            cursor += 1
        return None

    def _read_script_notation_at_mark(
        self, text: str, mark_cursor: int
    ) -> tuple[int, int, ScriptParseResult] | None:
        """从 _/^ 标记位置向左读取底数、向右读取脚标链。"""
        is_escaped_mark = self._is_escaped_char(text, mark_cursor)
        base_end = mark_cursor - 1 if is_escaped_mark else mark_cursor
        base_info = self._read_base_name_before(text, base_end)
        if base_info is None:
            return None
        start, base, is_escaped_base = base_info

        if is_escaped_mark:
            operand, end = self._read_script_operand(text, mark_cursor + 1)
            if operand is None:
                operand = ""
                end = mark_cursor + 1
            escaped_base = base + text[mark_cursor] + operand
            parsed = ScriptParseResult(
        ...
        while cursor < len(text) and text[cursor] in {"_", "^"}:
            if self._is_escaped_char(text, cursor):
                break

            script_mark = text[cursor]
            operand, next_cursor = self._read_script_operand(text, cursor + 1)
            if operand is None:
                return None
            scripts.append(ScriptOperand(script_mark, operand))
            if script_mark == "_":
        ...
        return start, cursor, parsed

    def _read_base_name_before(
        self, text: str, base_end: int
    ) -> tuple[int, str, bool] | None:
        """从脚标标记左侧向前读取变量底数名称。"""
        cursor = base_end
        while cursor > 0 and self._is_base_part_char(text[cursor - 1]):
            cursor -= 1

        if cursor == base_end or not self._is_base_start_char(text[cursor]):
            return None

        if cursor > 0 and text[cursor - 1] == "\\":
            slash_cursor = cursor - 1
            if not self._is_base_boundary_before(text, slash_cursor):
                return None
            return slash_cursor, text[cursor:base_end], True

        if not self._is_base_boundary_before(text, cursor):
            return None
        return cursor, text[cursor:base_end], False
    ...
    def _build_html_script(self, parsed: ScriptParseResult) -> str:
        ...
        for script in parsed.scripts:
            tag_name = "sub" if script.mark == "_" else "sup"
            rendered += f"<{tag_name}>{script.value}</{tag_name}>"
        return rendered
    ...
    def _build_html_script_parts(self, parsed: ScriptParseResult) -> list[HtmlPart]:
        ...
        for script in parsed.scripts:
            tag_name = "sub" if script.mark == "_" else "sup"
            script_element = etree.Element(tag_name)
            script_element.text = script.value
            parts.append(script_element)
        return parts
    ...
    def _plain_script_suffix(self, parsed: ScriptParseResult) -> str:
        ...
        suffix = ""
        for script in parsed.scripts:
            suffix += f"{script.mark}{script.value}"
        return suffix
    ...
    def _is_base_part_char(self, char: str) -> bool:
        """判断字符是否可作为变量底数的非首字符。"""
        return self._is_base_start_char(char) or char.isdigit() or char == "'"
