...
def _slice_to_ir(node: ast.AST, unparse: Unparser) -> ir.MathNode:
    ...
    if isinstance(node, ast.Tuple):
        tuple_parts: list[ir.MathNode] = []
        for idx, elt in enumerate(node.elts):
            if idx:
                tuple_parts.append(ir.mo(","))
            tuple_parts.append(_slice_to_ir(elt, unparse))
        return ir.mrow(tuple_parts)

    if isinstance(node, ast.Slice):
        slice_parts: list[ir.MathNode] = []
        if node.lower is not None:
            slice_parts.append(_slice_to_ir(node.lower, unparse))
        slice_parts.append(ir.mo(":"))
        if node.upper is not None:
            slice_parts.append(_slice_to_ir(node.upper, unparse))
        if node.step is not None:
            slice_parts.append(ir.mo(":"))
            slice_parts.append(_slice_to_ir(node.step, unparse))
        return ir.mrow(slice_parts)
