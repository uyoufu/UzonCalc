...
const FORCED_BREAK_VALUES = new Set(['always', 'page', 'left', 'right', 'recto', 'verso'])
...
function hasForcedPageBreakBefore(element: Element): boolean {
  ...
  return (
    isForcedPageBreakValue(resolveStyleValue(style, 'breakBefore', 'break-before')) ||
    isForcedPageBreakValue(resolveStyleValue(style, 'pageBreakBefore', 'page-break-before'))
  )
}
...
function hasForcedPageBreakAfter(element: Element): boolean {
  const style = resolveComputedStyle(element)
  return (
    isForcedPageBreakValue(resolveStyleValue(style, 'breakAfter', 'break-after')) ||
    isForcedPageBreakValue(resolveStyleValue(style, 'pageBreakAfter', 'page-break-after'))
  )
}
...
function hasAvoidPageBreakInside(element: Element): boolean {
  ...
  const style = resolveComputedStyle(element)
  return (
    isAvoidPageBreakValue(resolveStyleValue(style, 'breakInside', 'break-inside')) ||
    isAvoidPageBreakValue(resolveStyleValue(style, 'pageBreakInside', 'page-break-inside'))
  )
}
...
function resolveElementMetric(element: Element, content_element: Element): ElementMetric {
...
function collectContentHeadings(content_element: Element): HTMLElement[] {
  return Array.from(content_element.querySelectorAll<HTMLElement>('h2, h3, h4, h5, h6')).filter(
    (heading) => !heading.closest('#toc') && Boolean(heading.id)
  )
}
...
function applyColumnBreakStyles(source_element: HTMLElement, cloned_element: HTMLElement): void {
...
function createMeasuredContentClone(content_element: Element): HTMLElement {
  ...
  const source_elements = Array.from(content_element.querySelectorAll<HTMLElement>('*'))
  const cloned_elements = Array.from(cloned_content.querySelectorAll<HTMLElement>('*'))
...
function resolveKeepWithFollowingPageNumber(
  ...
  for (
    ...
    if (next_fragment_metric && next_fragment_metric.pageNumber === current_page_number + 1) {
...
function isVisibleFlowElement(element: Element, content_element: Element): element is HTMLElement {
  return (
...
function collectFlowElements(content_element: Element): FlowElement[] {
  return Array.from(content_element.children)
    .filter((element) => isVisibleFlowElement(element, content_element))
    .map((element) => ({
      element,
...
function writePageNumbers(page_numbers: PageNumberMap): void {
  page_numbers.forEach((page_number, heading_id) => {
    const toc_page_element = document.querySelector(`[data-heading-id="${heading_id}"]`)
...
function calculateStablePageNumbers(content_element: Element): void {
