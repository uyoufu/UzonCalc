...
class FakeDocument {
  ...
  querySelector(selector: string): FakeElement | null {
    if (selector === '.content') {
      return this.elements.find((element) => element.classList.contains('content')) ?? null
    }
...
function installFakeDocument(elements: FakeElement[], tocPages: Record<string, FakeElement>): void {
  ;(globalThis as { document: unknown }).document = new FakeDocument(elements, tocPages)
