...
class FakeElement {
  ...
  closest(selector: string): FakeElement | null {
    ...
    while (currentElement) {
      ...
      if (selector.startsWith('.') && currentElement.classList.contains(selector.slice(1))) {
        return currentElement
      }
...
class FakeDocument {
  ...
  querySelectorAll(selector: string): FakeElement[] {
    ...
    if (selector === 'h2, h3, h4, h5, h6') {
      return elements.filter((element) => ['H2', 'H3', 'H4', 'H5', 'H6'].includes(element.tagName))
    }
    if (selector === '.uz-outline-link') {
      return elements.filter((element) => element.classList.contains('uz-outline-link'))
    }
    if (selector === '.uz-outline-title') {
      return elements.filter((element) => element.classList.contains('uz-outline-title'))
    }
