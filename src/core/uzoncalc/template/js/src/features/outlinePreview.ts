import { collectDocumentHeadings, type DocumentHeadingItem } from './headingCollector'
...
/** 同步按钮图标和辅助说明。 */
function updateToggleButtonState(button: HTMLButtonElement, isExpanded: boolean): void {
...
function saveOutlineExpanded(isExpanded: boolean): void {
  try {
    window.localStorage?.setItem(OUTLINE_EXPANDED_STORAGE_KEY, String(isExpanded))
  } catch {
...
function createOutlinePanel(
  ...
  items.forEach((item) => nav.appendChild(createOutlineLink(item, handleActivateHeading)))
...
/** 刷新大纲链接的当前高亮状态。 */
function updateActiveOutlineLink(items: DocumentHeadingItem[], forceHeadingId?: string): void {
  const activeHeadingId = forceHeadingId ?? resolveActiveHeadingId(items)
  document.querySelectorAll<HTMLAnchorElement>('.uz-outline-link').forEach((link) => {
