...
describe("figure label styles", () => {
  test("模板样式包含图表标题和居中容器样式", () => {
    ...
    expect(templateStyles).toContain("a[data-uzoncalc-label-ref]");
    expect(templateStyles).toContain("cursor: pointer");
    expect(templateStyles).toContain("a[data-uzoncalc-label-ref]:hover");
    expect(templateStyles).toContain("a[data-uzoncalc-label-ref]:focus-visible");
