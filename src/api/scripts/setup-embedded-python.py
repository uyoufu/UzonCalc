...
def configure_python_path_file(embed_dir: Path) -> None:
    ...
    new_content = ["import site" if line == "#import site" else line for line in content]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Setup embedded Python for the UzonCalc API portable package.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if cache_python_exe.exists() and not embedded_python_version_matches(cache_python_exe, args.python_version):
        write_info(f"Cached embedded Python version does not match {args.python_version}. Removing cache.")
        remove_directory_if_exists(cache_dir)

    if cache_python_exe.exists():
        write_info("Found cached embedded Python environment. Copying to target directory...")
        remove_directory_if_exists(embed_dir)
        copy_directory_contents(cache_dir, embed_dir)
        python_exe = embed_dir / "python.exe"
        cache_hit = True
        write_info("Cached environment copied. Local workspace packages will still be refreshed.")
    ...
    if cache_hit:
        ...
        if embedded_python_version_matches(python_exe, args.python_version):
            ...
            write_info("Skipping download and extraction. Continuing with configuration and dependency installation.")
        else:
            write_info(f"Existing embedded Python is not {args.python_version}. Rebuilding: {embed_dir}")
            remove_directory_if_exists(embed_dir)
    ...
    if not cache_hit and not python_exe.exists():
        ...
        try:
            ...
            print(f"Please manually download and extract to: {embed_dir}")
    ...
    if not embedded_python_version_matches(python_exe, args.python_version):
        raise SetupError(f"Embedded Python version is not {args.python_version}: {python_exe}")
    ...
    if test_pip_available(python_exe):
        ...
        try:
            ...
            if not test_pip_available(python_exe):
                raise SetupError("pip still cannot run through python -m pip after installation.")
    ...
    if cache_hit:
        write_info("Using cached environment. Skipping third-party dependency installation.")
    else:
        if requirements_file.exists():
            ...
            try:
                pip_install(python_exe, ["install", "-r", str(filtered_requirements_file)])
            except Exception as exc:  # noqa: BLE001 - preserve the original failure details.
            ...
            print("  requirements.txt was not found. Skipping API dependency installation.")

        try:
            write_info("Install core package third-party dependencies from local source...")
            pip_install(python_exe, ["install", "--no-build-isolation", f"{CORE_PROJECT_ROOT}[all]"])
            run_command(
    ...
    try:
        pip_install(python_exe, ["install", "--no-build-isolation", "--no-deps", "--force-reinstall", str(CORE_PROJECT_ROOT)])
        pip_install(python_exe, ["install", "--no-build-isolation", "--no-deps", "--force-reinstall", str(PROJECT_ROOT)])
    except Exception as exc:  # noqa: BLE001 - preserve the original failure details.
    ...
    run_command([str(python_exe), "-m", "pip", "check"], failure_message="Python dependency consistency check failed.")

    write_info("=========================================")
    write_info("Embedded Python setup complete.")
    write_info("=========================================")
    print()
    print(f"Python path: {embed_dir}")
    print(f"Python executable: {python_exe}")
    print()
    write_info("Next steps:")
    print(f"  1. Test Python: {python_exe} --version")
    print(f"  2. Run app: {python_exe} main.py")
    print("  3. Keep the embedded Python environment under dist when distributing")
    print()
    return 0
