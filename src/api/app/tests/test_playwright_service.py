...
def configure_storage_state_path(monkeypatch, tmp_path: Path) -> Path:
    """将 storage_state 文件定位到临时目录，避免测试污染真实数据。"""
    storage_state_path = tmp_path / "data" / "playwright" / "storage_state.json"
    monkeypatch.setattr(
        playwright_service, "_STORAGE_STATE_PATH", storage_state_path, raising=False
    )
    return storage_state_path


async def start_fake_service(monkeypatch) -> FakePlaywrightFactory:
    """重置服务状态并注入测试用 Playwright 工厂。"""
    await playwright_service.close_playwright_service()
...
def test_allocate_page_returns_page_and_saves_state_in_background(monkeypatch, tmp_path):
    ...
    async def run_test():
        storage_state_path = configure_storage_state_path(monkeypatch, tmp_path)
        factory = await start_fake_service(monkeypatch)

        async with playwright_service.allocate_page() as first_page:
        ...
        async with playwright_service.allocate_page() as second_page:
...
def test_allocate_page_does_not_wait_background_storage_save(monkeypatch, tmp_path):
    ...
    async def run_test():
        configure_storage_state_path(monkeypatch, tmp_path)
        factory = await start_fake_service(monkeypatch)

        async with playwright_service.allocate_page():
...
def test_close_playwright_service_releases_context_browser_and_playwright(
...
def test_close_playwright_service_waits_background_storage_save(monkeypatch, tmp_path):
    ...
    async def run_test():
        ...
        close_task = asyncio.create_task(playwright_service.close_playwright_service())
        await asyncio.sleep(0)
...
def test_page_context_close_ignores_already_closed_page():
    """页面已关闭时，页面关闭操作不应抛出异常。"""
    ...
    async def run_test():
        ...
        await playwright_service._close_page(page)
