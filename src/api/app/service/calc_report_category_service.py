...
async def list_categories(
    ...
    rows = (
        await session.execute(
            ...
            .order_by(CalcReportCategory.sortOrder, CalcReportCategory.id)
        )
    ).all()
...
async def create_category(
    ...
    maximum = await session.scalar(
        select(func.max(CalcReportCategory.sortOrder)).where(
            CalcReportCategory.userId == user_id,
            CalcReportCategory.deletedAt.is_(None),
        )
    )
...
async def reorder_categories(
    ...
    for order in orders:
        categories[order.categoryOid].sortOrder = order.sortOrder
    await session.commit()
