...
export interface ILowCodeField {
  ...
  ) => Promise<IFunctionResult> | IFunctionResult // 验证函数
  parser?: (value: any) => any // 解析函数,在返回时，对数据进行转换
  visible?: ((allValues: Record<string, any>) => boolean) | boolean | string // 若存在，则 true 显示，否则隐藏；字符串由计算报告适配层转换为函数
  onChanged?:
    | ((
        value: any,
        oldValue: any,
        allValues: Record<string, any>,
        allFields: Array<ILowCodeField>
      ) => void | Promise<void>)
    | string // 字段值变化时执行；字符串由计算报告适配层转换为函数
