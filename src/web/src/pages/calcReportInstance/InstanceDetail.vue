...
import { ExecutionSourceType, type CalcExecution, type CalcInstance } from 'src/api/calc/types'
...
async function onRecalculate(): Promise<void> { if (!instance.value) return; isExecuting.value = true; try { const source = instance.value.sourceVersion ? { type: ExecutionSourceType.Version, versionName: instance.value.sourceVersion } : { type: ExecutionSourceType.Workspace }; const response = await startExecution({ reportOid: instance.value.reportOid, source, defaults: instance.value.defaults, isSilent: true }); execution.value = adaptExecutionFields(response.data) } finally { isExecuting.value = false } }
